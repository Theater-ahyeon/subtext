...
### Added —— 关系图谱（跨人物）
- 新页面「关系图谱」：从各对象的**理解卡条目**自动提取第三方人物与关系（TA 的父亲、闺蜜小林、同事……），与有档案的对象合并为力导向关系图（原味 SVG 实现，无依赖）
- **实时更新**：任何条目/档案变更自动置"待更新"标记，图谱页一键（或进入时提示）从理解卡重建；每条关系带证据引用
- 用户手动添加的关系（来源=user）在重建时**永远保留**；节点可拖拽整理布局，点击查看关系详情与氛围（亲近/中性/紧张/复杂）
- mock/降级模式使用称呼词启发式提取（诚实标注为弱提取）

## [1.2.5] - 2026-08-31

### Added —— 突发消息解读（未知因素假说分析）
- TA 在真实世界生活，很多事用户并不知道；突然发来的消息可能是这些未知因素导致的。深度分析新增第三模式「突发消息解读」：粘贴消息原文 + 背景补充 →
  - 字面解读 → 已知因素解释（理解卡/动态状态/相关往事优先）→ **未知因素假说**（2~4 条，各带可能性 高/中/低、支持线索、领域、自然求证方式）→ 建议回应方式 → 提醒
- 假说可一键「转为想多了解的」（理解卡空白层，进入对照复盘验证队列）或「记为动态状态」
- 纪律：已知解释优先；无线索支撑的假说不列；假说用于温和求证而非试探监视；禁临床标签
