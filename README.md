...
> 难开口的话，先演练一遍。为重要对话做练习的本地工具：整理你对 TA 的了解，和「TA 的模拟」把那段话先说一遍，再用真实反馈修正理解。**本地优先，只辅助理解与表达，不提供操控。**
...
2. **和「TA 的模拟」演练** —— 选一个场景开始演练。模拟由理解卡与事件记忆生成：卡越准、记越多，模拟越接近。TA 可以沉默、追问、拒绝、继续过自己的日子。
3. **写下预判 → 对照现实** —— 演练结束前写下预判（2~4 个带概率的心理假设）；拿到 TA 的真实反应后粘贴回来，AI 做**差异分析**（命中/事实错/材料缺/性情错/表达偏/**扮演偏差**），对理解卡做**可撤销的最小修正**。修正幅度按证据强度分级（学习率 α）：错得越离谱改得越多，单次命中只微调——防止过拟合于偶然。
4. **事件记忆** —— 每场演练复盘和每次现实对照会自动提取「发生过什么」存成事件段（向量化，本地优先）。下次开场时，相关往事会自动注入，让模拟记得你们之间发生过什么。
...
| **演练** | 场景模板（日常/约会/冷战/道歉/重要谈话/请求）或自定义。分"情境设定"（TA 可感知）与"演练目标"（仅复盘可见）。相关往事自动注入；结束生成复盘报告：六个观察指标 + 你的沟通复盘 + 下轮建议 + 现实验证清单 |
| **对照复盘** | 预判冻结 → 现实对照（原话优先，自动存证 + 入事件记忆）→ 差异分析（含扮演偏差归类）→ 卡片最小修正（学习率 α 分级，可一键撤销）。统计：Top1/Top2 命中率、Brier 校准、对照完成率 |
| **事件记忆** | 演练与现实事件段向量化存储；可查看、可单删、可清空、可重建索引；换 Embedding 模型后自动提示重建 |
| **想多了解的** | 理解卡空白 + 用户陈述待验证 + 访谈待确认 → "下次可以了解 TA 什么" |
| **状态栏** | 游戏风格的人物速览：档案槽位（性别/生日/职业/家庭/爱好/喜爱的食物等，AI 提取 + 手动编辑）、成长阶段（初见→同频）、五条数值条、四层分布、最近动态与演练 |
| **深度分析** | 对这个人或某个场景的完整分析：本地统计（素材月度分布/演练/预判命中/记忆）+ AI 整合报告；场景推演给出反应路径（带可能性）、相关往事、最优策略，可一键转入演练；**突发消息解读**：TA 突然的消息可能源于你不知道的事——已知解释优先，未知因素给出可求证的假说（高/中/低可能 + 支持线索 + 求证方式），可一键转为待验证项 |
...
- **本地优先**：档案、证据、演练、访谈、事件记忆全部数据只存在本机数据目录，删除档案即全部删除。
...
src/
  main/          # 主进程（无 Electron 依赖的纯 Node 模块 + main.js IPC 编排）
    ...
    pipeline.js  #   业务流：归纳→演练→预判→现实对照→差异分析→撤销、访谈
