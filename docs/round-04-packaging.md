...
`build/make-icon.ps1`（PowerShell + System.Drawing 纯代码生成）：墨底圆角方块 + 琥珀-玉色双生水滴（生境意象），512×512 PNG，electron-builder 自动转换为 ico/icns。
...
**产物**（v1.0.0）：
- `Habitat Sandbox Setup 1.0.0.exe`（NSIS 安装向导，桌面/开始菜单快捷方式，可选安装目录）
- `Habitat Sandbox-1.0.0-win.zip`（便携版）

## 其他配置

- NSIS 中文快捷方式/卸载显示名（生境沙盒）
