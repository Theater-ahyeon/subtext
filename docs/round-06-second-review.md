...
| 3 | **extractJson 多围栏取错优先级**：模型先回显格式示例再给真实结果时，归因/预测静默采纳示例占位数据写入卡片与统计 | 候选从后向前尝试（真实结果惯例在后）；新增多围栏回归测试 |
...
| 7 | 撤销归因后重交反馈 → `loopCompletion` 200%，且撤销不清 feedback 关联 | undo 时 feedback.predictionId 置 null 退出闭环分子；loopCompletion 封顶；统计导出 `linkedFeedbacks`，UI 标签用真值 |
...
| 13 | **访谈 fact 绕开证据链纪律**：`kind=fact` 以 `[事实]` 身份进卡（孪生被要求"对事实保持连续"） | 用户陈述一律以 `inference` 落库，与归纳器同一纪律 |
...
| 28 | 直录归因（无预测单）的提示词残留"引用预测假设编号"语境 | direct-mode 独立规则（verdict 仅错误四类，对照生境卡条目） |
